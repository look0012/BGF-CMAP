...
def MyEnlarge(x0, y0, width, height, x1, y1, times, mean_fpr, mean_tpr, thickness=1, color = 'blue'):
    def MyFrame(x0, y0, width, height):
        ...
        plt.plot(x1, yk, color='k', linestyle=':', lw=20, alpha=1)
        plt.plot(xk, y1, color='k', linestyle=':', lw=20, alpha=1)
        plt.plot(xkn, yk, color='k', linestyle=':', lw=20, alpha=1)
        plt.plot(xk, ykn, color='k', linestyle=':', lw=20, alpha=1)
    ...
    xp = np.linspace(x0 + width, x1, num=30)
    yp = np.linspace(y0, y1 + height2, num=30)
    plt.plot(xp, yp, color='k', linestyle=':', lw=3, alpha=1)
    ...
    plt.plot(XDottedLine, YDottedLine, color=color, lw=3, alpha=1)
    return
...
while counter0 < 5:
    ...
    plt.plot(recall, precision, lw=2, alpha=0.8, color=colorlist[i],
...
plt.locator_params("x", nbins=10)  # x轴分为5分的刻度值
...
plt.plot([1, 0], [0, 1], color='navy', lw=3, linestyle='--')
plt.legend(bbox_to_anchor=(0.78, 0.78), fontsize=15)
...
plt.text(0.93, 0.9, s='B', ha='center', **style)
